...
sample.block.t1.normal.walk.and.lambdas = function(t1.prev, lambda0.prev, lambda1.prev, beta, sigma, t0, t2, y.prev, dates) {
  ...
  print(paste(t1.prev, t0, t2, t1.proposal))
  ...
  #calculate acceptance probability
  accept.prob = min(exp(log.joint.conditional.t1.lambda0.lambda1(t1.proposal, lambda0.proposal, lambda1.proposal, y.proposal) - 
                          log.joint.conditional.t1.lambda0.lambda1(t1.prev, lambda0.prev, lambda1.prev, y.prev)), 1)
  print(accept.prob)
  # get a uniform random number between 0 and 1
  u = runif(1)
  # Return new propsals if u < acceptance probability
...
df = block.mcmc(n, K, df.coal$date, sigma, block.proposal = "t1.lambda0.lambda1")
plot(df$iter, df$t1, type ="l")
df$t1
```

```{r}
# Implements a block mcmc formula for approximating the posterior distribution of the parameters of interest
block.mcmc = function(n, K, dates, sigma, 
                      t1.initial = runif(1, dates[1], dates[length(dates)]),
                      beta.initial = 1 / rgamma(1, shape = 2, rate = 1),
                      lambda0.initial = 1 / rgamma(1, shape = 3, scale = beta.initial),
                      lambda1.initial = 1 / rgamma(1, shape = 3, scale = beta.initial),
                      block.proposal = "t1.lambda0.lambda1") {
  "
  Input:
    -n: number of useful samples after the burn-in period wanted
    -K: length of the burn-in period
    ...
    -lambda1.initial: the initial value of lambda1. If not provided it is sampled from the
    prior given beta.initial
    -blokc.proposal: which block proposal is to be used in the algorithm
  ...
  n.accepted.block = 0 # keep track of number of accepted block proposals
  n.accepted.t1 = 0 # keep track of number of accepted t1 proposals (if applicable)
  for (i in 2:(n+K)) {
    if (block.proposal == "t1.lambda0.lambda1") {
      # sample new beta value from full conditional distribution
      beta.values[i] = sample.beta.full.conditional(lambda0.values[i-1], lambda1.values[i-1])
      
      # simulate new t1, lambda0 and lambda1 value. Get corresponding new y value
      new.block.variables = 
        sample.block.t1.normal.walk.and.lambdas(t1.values[i-1], lambda0.values[i-1], lambda1.values[i-1],
                                                beta.values[i], sigma, t0, t2, y, dates)
      ...
      n.accepted.block = n.accepted.block + new.block.variables$accepted
      n.accepted.t1 = NA # not applicable as t1 is included in block sampling
    }
  }
  ...
  return(list(t1 = t1.values, beta = beta.values, lambda0 = lambda0.values, lambda1 = lambda1.values,
              ...
              percent.t1.accepted = n.accepted.t1 / (n + K - 1),
              percent.block.accepted = n.accepted.block / (n + K -1)))
}
```
...
gibbs_sampling = function(precision_matrix, pi_f_gtheta, n_samples){
  theta = 1
  #Q = matrix(nrow = n_samples, ncol = n_samples) denne ble feil
  # for i for og denne
}
```
